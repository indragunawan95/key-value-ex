defmodule KeyValueEx.Application do
  # See https://hexdocs.pm/elixir/Application.html
  # for more information on OTP Applications
  @moduledoc false

  use Application

  @impl true
  def start(_type, _args) do
    children = [
      KeyValueExWeb.Telemetry,
      # KeyValueEx.Repo,
      {DNSCluster, query: Application.get_env(:key_value_ex, :dns_cluster_query) || :ignore},
      {Phoenix.PubSub, name: KeyValueEx.PubSub},
      # Start a worker by calling: KeyValueEx.Worker.start_link(arg)
      # {KeyValueEx.Worker, arg},
      # Start to serve requests, typically the last entry
      KeyValueExWeb.Endpoint
    ]

    # See https://hexdocs.pm/elixir/Supervisor.html
    # for other strategies and supported options
    opts = [strategy: :one_for_one, name: KeyValueEx.Supervisor]
    Supervisor.start_link(children, opts)
  end

  # Tell Phoenix to update the endpoint configuration
  # whenever the application is updated.
  @impl true
  def config_change(changed, _new, removed) do
    KeyValueExWeb.Endpoint.config_change(changed, removed)
    :ok
  end
end
